...
using System.Text;
using TencentWeiboSDK.Hammock.Attributes.Specialized;
using TencentWeiboSDK.Hammock.Caching;
using TencentWeiboSDK.Hammock.Extensions;
using TencentWeiboSDK.Hammock.Validation;
using TencentWeiboSDK.Hammock.Web.Mocks;
...
namespace TencentWeiboSDK.Hammock.Web
{
 ...
 "HTTP/v1.1";
        ...
        private long WriteMultiPartImpl(bool write, IEnumerable<HttpPostParameter> parameters, string boundary, Encoding encoding, Stream requestStream)
        {
            ...
            //written += WriteLine(write, encoding, requestStream, string.Format("Content-type: multipart/form-data, boundary={0}", boundary));
...
#endif
                            {
                                ...
                                else
                                {
                                    ...
                                    using (var br = new BinaryReader(fs))
                                    {
                                        ...
                                        {
                                            //var buffer = br.ReadBytes(0x2000);
                                            ...
                                            byte[] buffer = br.ReadBytes(0x2000);
                                            str += buffer.ToString();
                                            requestStream.Write(buffer, 0, buffer.Length);
                                            written += buffer.Length;
                                            fileWritten += buffer.Length;
                                            PostProgressEventArgs args2 = new PostProgressEventArgs();
                                            args2.FileName = parameter.FileName;
                                            args2.BytesWritten = written;
                                            args2.TotalBytes = fs.Length;
                                            PostProgressEventArgs args = args2;
                                            this.OnPostProgress(args);
                                        }
                                    }
                                }
                            }
